...
def test_generate_series_ariza_yokken_hepsi_normal():
    ...
    rows = generate_series(hat, "T-01", "V1", "A1", forced_episodes=[], hareket=hareket,
...
def test_kisa_segment_uretimi_cokmez():
    ...
    import datetime
    df, durumlar = bir_segment_uret(hatlar, rng, datetime.datetime(2026, 1, 1, 6, 0, 0),
                                    None, kusurlar, n_steps=5)
    assert len(df) > 0
